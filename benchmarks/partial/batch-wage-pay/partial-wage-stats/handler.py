...
def handle(req):
    ...
    with ThreadPoolExecutor(max_workers=len(manifest)) as executor:
        for obj in manifest:
            if obj != "raw/":
                fs.append(executor.submit(requests.get, url = 'http://' + OF_Gateway_IP + ':' + OF_Gateway_Port + '/function/partial-wage-sum-avg-merit-write', data = json.dumps({'total': total, 'base': base, 'merit': merit, 'operator': obj})))
